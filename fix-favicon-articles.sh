#!/bin/bash
# Script pour ajouter le favicon "C" à tous les articles HTML (compatible macOS)

echo "🔧 === AJOUT DU FAVICON 'C' À TOUS LES ARTICLES (Version macOS) ==="
echo ""
...
find articles/html -name "*.html" | while read -r file; do
    ...
    if ! grep -q "favicon.ico" "$file"; then
        ...
        if grep -q "</title>" "$file"; then
            # Créer un fichier temporaire avec le favicon ajouté (syntaxe BSD sed compatible macOS)
            sed -e '/^<title>.*<\/title>$/a\
    <link rel="icon" type="image/x-icon" href="../../favicon.ico">\
    <link rel="apple-touch-icon" href="../../favicon.ico">' "$file" > "${file}.tmp" && mv "${file}.tmp" "$file"
